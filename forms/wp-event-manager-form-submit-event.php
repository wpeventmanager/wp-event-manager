	...
	protected function validate_fields( $values ) {
	    ...
	    foreach ( $this->fields as $group_key => $group_fields ) {     	      
    	    ...
    	    if(isset($group_fields['event_online'] ) ) {
    			if(isset($values[$group_key]['event_online']) && $values[$group_key]['event_online']=='yes') {
					...
					unset($values[$group_key]['event_venue_name']);
					unset($values[$group_key]['event_address']);
					unset($values[$group_key]['event_pincode']);
					unset($values[$group_key]['event_location']);
					unset($values[$group_key]['event_country']);
				}
			}
			if(isset($group_fields['event_ticket_options']) ) {
				if(isset($values[$group_key]['event_ticket_options']) && $values[$group_key]['event_ticket_options']=='free') {	
					$group_fields['event_ticket_price']['required']=false;
					unset($values[$group_key]['event_ticket_price']);
				} 			
			}
	        ...
	        foreach ( $group_fields as $key => $field ) {
				...
				if ( ! empty( $field['taxonomy'] ) && in_array( $field['type'], array( 'term-checklist', 'term-select', 'term-multiselect' ), true ) ) {
					...
					if ( is_array( $values[ $group_key ][ $key ] ) ) {
						$check_value = array_map( 'absint', $values[ $group_key ][ $key ] );
					} else {
						$check_value = empty( $values[ $group_key ][ $key ] ) ? [] : [ absint( $values[ $group_key ][ $key ] ) ];
					}
