<?php if(empty($field_key))
	...
	?>
<tr data-field-type="<?php echo $field['type']; ?>">
	...
	<td class="field-options">
		<?php
			if(isset($field['options'])){
				...
				$options = '';	
		?>
		<input type="text" class="input-text placeholder" name="<?php echo $group_key;?>[<?php echo $field_key;?>][placeholder]" value="<?php if(isset($field['placeholder'])) printf( esc_html__( '%s', 'wp-event-manager' ), esc_attr( stripslashes( $field['placeholder'])) );?>" placeholder="<?php _e( 'N/A', 'wp-event-manager' ); ?>" />
	...
	<?php if( !in_array($field_key, $disbled_fields) ) : ?> 
		<input type="checkbox" name="<?php echo $group_key;?>[<?php echo $field_key;?>][admin_only]" value="1" <?php checked( ! empty( $field['admin_only'] ), true ); ?> />
	<?php endif; ?>
	</td>
...
if( isset($field['type']) && $field['type'] == 'group' )
{
	...
	<tr class="group">
		<td colspan="10">
			<table class="widefat child_table" id="<?php echo $field_key; ?>">
				<thead>
					<tr>
						<th width="1%">&nbsp;</th>
				...
				<tbody class="child-form-fields" data-name="<?php echo $group_key;?>[<?php echo $field_key;?>][fields]" data-field="<?php
								ob_start();
