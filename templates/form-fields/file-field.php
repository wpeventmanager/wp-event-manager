...
$wpem_allowed_mime_types = array_keys(!empty($field['allowed_mime_types']) ? $field['allowed_mime_types'] : get_allowed_mime_types());
$wpem_field_name         = isset($field['name']) ? $field['name'] : $key;
$wpem_field_name         .= !empty($field['multiple']) ? '[]' : '';
...
<div class="event-manager-uploaded-files">
	<?php if (!empty($field['value'])) : 
		if (is_array($field['value'])) : 
			foreach ($field['value'] as $wpem_value) : 
				wpem_get_event_manager_template('form-fields/uploaded-file-html.php', array('key' => $key, 'name' => 'current_' . $wpem_field_name, 'value' => $wpem_value, 'field' => $field)); 
			endforeach; 
		elseif ($wpem_value = $field['value']) :
			wpem_get_event_manager_template('form-fields/uploaded-file-html.php', array('key' => $key, 'name' => 'current_' . $wpem_field_name, 'value' => $wpem_value, 'field' => $field));
		endif;
	endif; ?>
</div>

<input type="file" class="<?php echo esc_attr(implode(' ', $wpem_classes)); ?>" attribute="<?php echo esc_attr(isset($field['attribute']) ? $field['attribute'] : ''); ?>" data-file_types="<?php echo esc_attr(implode('|', $wpem_allowed_mime_types)); ?>" <?php if (!empty($field['multiple'])) echo esc_attr('multiple'); ?> name="<?php echo esc_attr(isset($field['name']) ? $field['name'] : $key); ?><?php if (!empty($field['multiple'])) echo esc_attr('[]'); ?>" id="<?php echo esc_attr($key); ?>" placeholder="<?php echo empty($field['placeholder']) ? '' : esc_attr($field['placeholder']); ?>" />
