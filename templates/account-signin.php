<?php if (is_user_logged_in()) : ?>
	<div class="wpem-form-group ">
		...
		<div class="field account-sign-in wpem-alert wpem-alert-info"> <?php $user = wp_get_current_user();
																																		printf(wp_kses(__('You are currently signed in as <strong>%s</strong>.', 'wp-event-manager'), array('strong' => array())), $user->user_login);    		?> <a href="<?php echo apply_filters('submit_event_form_logout_url', wp_logout_url(get_permalink())); ?>"><?php _e('Sign out', 'wp-event-manager'); ?></a>
		</div>
	</div>
<?php else :
