...
<form action="<?php echo esc_url($action); ?>" method="post" id="submit-organizer-form" class="wpem-form-wrapper wpem-main event-manager-form" enctype="multipart/form-data">
    ...
    if (is_user_logged_in()) {
        ...
        $username = !empty($user->display_name) ? $user->display_name : $user->user_login;
         ...
         if (!isset($organizer_fields['organizer_name']['value']) && empty($organizer_fields['organizer_name']['value']) && $organizer_fields['organizer_name']['visibility'] != 0) {
            $organizer_fields['organizer_name']['value'] = $username;
        }
        if (!isset($organizer_fields['organizer_email']['value']) && empty($organizer_fields['organizer_email']['value']) && $organizer_fields['organizer_email']['visibility'] != 0) {
