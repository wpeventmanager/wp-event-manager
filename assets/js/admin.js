var Admin = function ()
{
    ...
    return {
        ...
        init: function ()
        {
            ...
            if (jQuery('input[data-picker="datepicker"]#_event_start_date').length > 0)
            {
                ...
                if (wp_event_manager_admin_js.show_past_date)
                {
                    ...
                    }).on('change', function ()
                    {
                      ...
                      //Fix the date issue (Now user can't select start date greater than end date)
                        //Author - Sandeep Kumar
                        
                        if (jQuery('#_event_start_date').val() > jQuery('#_event_end_date').val()) {
                            alert('Please add end date again! Because start date is greater than end date')
                            jQuery('#_event_end_date').val('');
                          
                          //border color is optional, you can keep or remove this code according to your need and underunderstanding 
                            jQuery('#_event_end_date').css('border', '1px solid #ff0000');
                        }
                        else {
                            jQuery('#_event_end_date').css('border', '1px solid #8c8f94');
                        }
          ...
          //Fix the time issue (Now user can't select start time greater than end time)
            //Author - Sandeep Kumar

            //Create own custom function for on change event for selecting time
            jQuery('#_event_end_time,#_event_start_time').on('change', function () {
                let eventStartDate = jQuery('#_event_start_date').val();
                let eventEndDate = jQuery('#_event_end_date').val();
                if (eventStartDate == eventEndDate) {
                    let eventStartTime = jQuery('#_event_start_time').val();
                    let eventEndTime = jQuery('#_event_end_time').val();
                    modifyEventTime(eventStartTime, eventEndTime);
                }
            });

            //Create functionality for comparing time with AM, PM
            function modifyEventTime(eventStartTime, eventEndTime) {
                eventStartTime = eventStartTime.split(" ");
                var time = eventStartTime[0].split(":");
                var stime = time[0];
                if (eventStartTime[1] == "PM" && stime < 12) stime = parseInt(stime) + 12;
                eventStartTime = stime + ":" + time[1] + ":00";

                eventEndTime = eventEndTime.split(" ");
                var time1 = eventEndTime[0].split(":");
                var etime = time1[0];
                if (eventEndTime[1] == "PM" && etime < 12) etime = parseInt(etime) + 12;
                eventEndTime = etime + ":" + time1[1] + ":00";

                if (eventStartTime != '' && eventEndTime != '') {
                    if (eventEndTime <= eventStartTime) {
                        alert('Please add end time again! Because start time is greater than end time')
                        jQuery('#_event_end_time').val('');
                      
                      //border color is optional, you can keep or remove this code
                        jQuery('#_event_end_time').css('border', '1px solid #ff0000');
                    }
                    else {
                        jQuery('#_event_end_time').css('border', '1px solid #8c8f94');
                    }
                }
            }
            //functionality end here
